    ...
    public static class BulldozeToolSystem_Patches
    {
        ...
        public static bool Prefix(BulldozeToolSystem __instance, ref bool __result)
        {
            ...
            for (int i = 0; i < nativeArray.Length; i++)
            {
                ...
                PrefabRef prefabRef;
                if ((entityManager.GetComponentData<Temp>(entity).m_Flags & TempFlags.Delete) != (TempFlags)0U && entityManager.TryGetComponent(entity, out prefabRef) && (!entityManager.HasComponent<SpawnableBuildingData>(prefabRef.m_Prefab) || (!entityManager.HasComponent<PloppableBuildingData>(prefabRef.m_Prefab) && entityManager.HasComponent<SignatureBuildingData>(prefabRef.m_Prefab))))
                {
                    flag = true;
                }
            }
